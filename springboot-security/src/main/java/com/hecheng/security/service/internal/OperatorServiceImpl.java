...
import com.hecheng.security.repository.internal.OperatorDao;
...
public class OperatorServiceImpl implements OperatorService {
  ...
  public OperatorEntity findByAccountAndStatus(String account, StatusType status) {
    ...
    return accountService.findOperatorByAccountAndStatus(account, status);
  }
  ...
  public OperatorEntity addOperator(OperatorEntity operator) {
    ...
    operator = operatorDao.saveAndFlush(operator);
    // accountService.create(operator, account);
    return operator;
  }
  ...
  public void updatePassword(String operatorId, String oldPass, String newPass,
    ...
    // 检查新密码格式是否正确
    Validate.matchesPattern(newPass, "[a-zA-Z\\d]{6,12}", "新密码长度必须为6-12个字符，大小写英文字符或数字！");
    // 修改管理员的密码和修改人信息并保存
