...
import com.vanda.oauth2.server.controller.model.ResponseCode;
import com.vanda.oauth2.server.controller.model.ResponseModel;
import com.vanda.oauth2.server.entity.UuidEntity;
...
public class BaseController {
  ...
  protected String getPrincipalAccount(Principal operator) {
    Validate.notNull(operator, "登录已过期，或者还未登录，请重新登录!");
    // 通过operator确定当前的登录者信息
    String account = operator.getName();
    Validate.notBlank(account, "用户不存在!");
    return account;
  }
  ...
  protected ResponseModel buildHttpReslutForException(Exception e) {
    ResponseCode code = ResponseCode._501;
    String errorMsg = "";
    if (e != null) {
      errorMsg = e.getMessage();
      // if(Objects.equal(errorMsg, "登录已过期，或者还未登录，请重新登录")||Objects.equal(errorMsg, "登录过期，请进行登录..."))
      // {
      // code=ResponseCode._1000;
      // }
    }
    LOG.error(errorMsg, e);
    ResponseModel result = new ResponseModel(new Date().getTime(), null, code, errorMsg);
    return result;
  }
  ...
  protected <T> ResponseModel buildHttpReslut(T entity) {
    ResponseModel result = new ResponseModel(new Date().getTime(), null, ResponseCode._200, null);
    if (entity == null) {
      return result;
    }
