...
import com.hecheng.curator.controller.model.ResponseCode;
import com.hecheng.curator.controller.model.ResponseModel;
import com.hecheng.curator.entity.BaseUuidEntity;
import com.hecheng.curator.entity.OperatorEntity;
import com.hecheng.curator.service.OperatorService;
...
public class BaseController {
   ...
   * 构造http返回信息
   * 
   * @see #filterProperties(BaseUuidEntity, String...)
   * @param properties
   * @return
   */
  protected <T extends BaseUuidEntity> ResponseModel buildHttpReslut(Iterable<T> page,
      String... properties) {
   ...
   * @return
   */
  protected <T extends BaseUuidEntity> ResponseModel buildHttpReslut(Collection<T> entities,
      String... properties) {
    ResponseModel result = new ResponseModel(new Date().getTime(), null, ResponseCode._200, null);
   ...
   * @return
   */
  protected <T extends BaseUuidEntity> ResponseModel buildHttpReslut(T entity,
      String... properties) {
    ResponseModel result = new ResponseModel(new Date().getTime(), null, ResponseCode._200, null);
   ...
   * 注意，进行属性引用去除的对象必须是BaseUuidEntity的子类示例对象
   * 
   * @param object 目标对象
