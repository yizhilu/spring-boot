package com.hecheng.core.advice;
...
import com.hecheng.core.configuration.CryptoProperties;

/**
 * 请求响应处理类<br>
 * 
 * 对加了@ApiEncryptAnno的方法的数据进行加密操作 <pre> 1. sever生成AES密钥(aesKey) 2.
 * sever使用client的RSA公钥对aesKey进行加密得到(encryptkey) 3. sever使用自己的RSA私钥对请求明文数据(json)进行数字签名 4.
 * sever将签名加入到请求参数中，然后转换为jsonStr格式 4. sever使用aesKey对json数据进行加密得到密文 5. encryptkey和密文返回给客户端
 * ============================================= 1. 对sever返回的数据，进行验签encryptkey
 * 为sever用client公钥加密后的AES密钥 2. 使用client私钥对encryptkey进行解密，得到aeskey 3. 使用aesKey对json数据进行解密得到明文(data)
 * <pre>
 * 
 * @author hc
 * 
